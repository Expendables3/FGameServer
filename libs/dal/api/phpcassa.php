...
class CassandraCF {
    ...
    const DEFAULT_SUBCOLUMN_TYPE = "BytesType";
    ...
    */
    ...
    public function getList($key, $super_column=NULL, $IdList) {
        $column_parent = new cassandra_ColumnParent();
        $column_parent->column_family = $this->column_family;
        $column_parent->super_column = $this->unparse_column_name($super_column, true);

        $predicate = new cassandra_SlicePredicate();
        $predicate->column_names = $IdList;
...
//        }
    ...
    public function remove($key, $column_name=null,$column = null) {
        $timestamp = CassandraUtil::get_time();
        ...
        if($this->is_super) {
            ...
            $column_path->column = $this->unparse_column_name($column, false);
        } else {
