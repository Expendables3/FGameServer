<?php
    ...
    foreach($confKey as $key =>$confId)
    {
        ...
        if(!empty($aa)) // key nay dang duoc luu tren he thong 
        {
            if(!DataProvider::delete($key,'ConfigItemCode'))
		    echo "$key=>$confId,\n";
        }       
    }
    /*
    $arr_conf_id = array(1,2,500001,500002,900000);
    
    foreach($arr_conf_id as $IdConfigGift )
    {
        DataProvider::delete(-111,'ConfigItemCode',$IdConfigGift);
    }
    */
