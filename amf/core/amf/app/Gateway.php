...
class Gateway {
	...
	function Gateway() {
		...
		if(AMFPHP_PHP5)
		{
			...
			include_once(ROOT_DIR . "/Execution/SNSExecutive.php");
	...
	function service() {
		...
		if(isset($GLOBALS["HTTP_RAW_POST_DATA"]) && $GLOBALS["HTTP_RAW_POST_DATA"] != "")
		{
			...
			if($this->outgoingMessagesFolder != NULL)
			{
				...
				file_put_contents($this->outgoingMessagesFolder . 
					$pieces[1] . '.' . substr($pieces[0], 2) . ".out", $output);
			}
		...
		{
			echo("<p> Gateway is conecting...</p>");
				 
			if(function_exists("amf_decode"))
			{
				echo("<p>SNS Gateway is conected " .
					($GLOBALS['amfphp']['native'] ? "OK" : "but not OK") .
					"</p>");
			}
